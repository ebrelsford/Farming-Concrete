...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        """Set quantity and units for all patches"""

        # Handle patches with areas
        for patch in orm.Patch.objects.filter(area__isnull=False):
            patch.quantity = patch.area
            patch.units = 'square feet'
            patch.save()

        # Handle patches with plants
        for patch in orm.Patch.objects.filter(quantity__isnull=True, plants__isnull=False):
            patch.quantity = patch.plants
            patch.units = 'plants'
            patch.save()


    def backwards(self, orm):
        """Unset quantity and units for all patches"""
        orm.Patch.objects.update(quantity=None, units=None)
    ...
    models = {
        ...
        u'cropcount.patch': {
            ...
            'quantity': ('django.db.models.fields.DecimalField', [], {'null': 'True', 'max_digits': '5', 'decimal_places': '2', 'blank': 'True'}),
            'recorded': ('django.db.models.fields.DateField', [], {'null': 'True', 'blank': 'True'}),
            'units': ('django.db.models.fields.CharField', [], {'max_length': '15', 'null': 'True', 'blank': 'True'}),
            'updated': ('django.db.models.fields.DateTimeField', [], {'auto_now': 'True', 'blank': 'True'}),
